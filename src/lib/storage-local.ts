...
function toUserProgressDTO(raw: any): UserProgressDTO {
  return {
    ...
    updatedAt: raw.updatedAt ?? raw.lastActiveDate ?? 0,
  };
}
 ...
 *
 * Bản cũ không nhận `userId` mà quét `Object.keys(localStorage)` tìm mọi khoá
 * `memory:*` rồi sửa khoá đầu tiên chứa id đó. Hai vấn đề: hàm ném lỗi ngay khi
 * chạy phía server (không có `localStorage`), và nó ghi được vào dữ liệu của
 * tài khoản khác trên cùng trình duyệt. Giờ nó nhận `userId` như mọi hàm còn
 * lại trong file, và bản remote sẽ lấy `userId` từ session chứ không từ client.
 */
...
/**
 * Ghi đè toàn bộ các kho của một người dùng bằng dữ liệu đã hợp nhất.
 *
 * Chỉ engine đồng bộ (src/lib/sync.ts) mới nên gọi. Ghi thẳng, không đóng dấu
 * lại `updatedAt`: mốc thời gian đi kèm bản ghi chính là thứ vừa quyết định
 * bên nào thắng, đóng dấu lại sẽ phá luôn kết quả hoà giải.
 */
export function replaceLocalCollections(
  userId: string,
  data: {
    texts: TextDTO[];
  ...
  if (typeof window === "undefined") return;
  setStore(`texts:${userId}`, data.texts);
  setStore(`vocab:${userId}`, data.vocabItems);
  setStore(`memory:${userId}`, data.memoryItems);
  setStore(`quizzes:${userId}`, data.quizQuestions);
  setStore(`shadows:${userId}`, data.shadowSessions);
...
/**
 * Xoá sạch mọi kho của một người dùng trên máy này.
 *
 * Liệt kê khoá thẳng ở đây thay vì quét `Object.keys(localStorage)` theo tiền
 * tố: quét sẽ vớ luôn dữ liệu của tài khoản khác dùng chung trình duyệt.
 */
export function clearLocalCollections(userId: string): void {
  if (typeof window === "undefined") return;
  const keys = [
    `texts:${userId}`,
    `vocab:${userId}`,
